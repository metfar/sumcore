...
`suminfo` reports three intentionally separate views:

- **HARDWARE**: architecture, CPU count, memory and Android audio properties
  when `termux-audio-info` is available;
- **SOFTWARE**: OS/kernel/Python, Termux, terminal, Pygame, PulseAudio sinks,
  available audio commands and installed Sum package versions;
- **SIMULATED**: the capabilities implemented by Sum itself, including its PCM
  format/sample rate, audio semantics, polyphony, key-release model and the
  current `sumaudiod` backend.
