...
def test_dependency_model():
    ...
    assert DEFAULT_ARCHITECTURE["sumTerminal"]["requires"] == ["sumFSA","sumIO","sumbash","sumGUI","sumKeyboard"];
...
def test_cli_version(capsys):
    ...
    assert exc.value.code==0; assert "sumcore 0.1.0a18" in capsys.readouterr().out;
...
def test_architecture_cli_version(capsys):
    ...
    import pytest;
    with pytest.raises(SystemExit) as exc: main(["--version"]);
    assert exc.value.code==0; assert "0.1.0a18" in capsys.readouterr().out;
